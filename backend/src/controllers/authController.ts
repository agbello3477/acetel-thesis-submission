...
export const login = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        res.json({ token, user: { id: user.id, full_name: user.full_name, email: user.email, role: user.role, program_type: user.program_type, matric_number: user.matric_number } });
    } catch (error) {
        console.error(error);
